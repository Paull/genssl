...
if [ -z "$1" ]; then
  echo
  echo 'Issue a client SSL certificate with Corn Root CA'
  echo
  echo "Usage: $0 <client> [<client2>] [<client3>] [<client4>] ..."
  echo '    <client>          The client name or person name, like "home_pc",'
  echo '                      you will get a certificate for the special device'
  echo '                      Multiple clients are acceptable'
  exit
fi
...
openssl x509 -req \
  ...
  -out "${DIR}/rsa_$1.crt" \
  -CA ./out/rsa_root.crt \
  -CAkey ./out/rsa_root.key -passin env:rootpass \
  -extfile ./ca.cnf -extensions usr_cert \
  -days ${days}
...
openssl x509 -req \
  ...
  -out "${DIR}/ec_$1.crt" \
  -CA ./out/ec_root.crt \
  -CAkey ./out/ec_root.key -passin env:rootpass \
  -extfile ./ca.cnf -extensions usr_cert \
  -days ${days}
